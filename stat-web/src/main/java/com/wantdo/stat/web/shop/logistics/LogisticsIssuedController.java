...
import com.wantdo.stat.service.shop.logistics.LogisticsIssuedService;
...
@Controller
@RequestMapping(value = "/logistics/issued")
public class LogisticsIssuedController {

    private static final String PAGE_SIZE = "10";

    @Autowired
    private LogisticsIssuedService logisticsIssuedService;
    ...
    public String normalList(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
        ...
        Page<Order> orders = logisticsIssuedService.SearchOrder(searchParams, pageNumber, pageSize, sortType);
        model.addAttribute("orders", orders);
        model.addAttribute("sortType", sortType);
        model.addAttribute("organizationId");
        // 将搜索条件编码成字符串，用于排序，分页的URL
        model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));

        return "shop/logistics/issuedList";
    }
    ...
    ResponseVo upload(MultipartFile uploadExcel) {
        return logisticsIssuedService.upload(uploadExcel);
    }
    ...
    public @ResponseBody void download(HttpServletResponse response){
        ...
        ExcelVo excelVo = logisticsIssuedService.download();
    ...
    public String updateForm(@PathVariable("id") Long id, Model model) {
        model.addAttribute("order", logisticsIssuedService.getOrder(id));
        model.addAttribute("action", "update");
        return "shop/logistics/issuedForm";
    }
    ...
    public String update(@Valid @ModelAttribute("order") Order order,
        ...
        logisticsIssuedService.updateOrder(order);
        redirectAttributes.addFlashAttribute("message", "更新订单成功");
        return "redirect:/logistics/issued/";
    }
